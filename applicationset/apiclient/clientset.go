...
type Clientset interface {
	NewApplicationSetClient() (io.Closer, applicationset.ApplicationSetServiceClient, error)
}
...
func (c *clientSet) NewApplicationSetClient() (io.Closer, applicationset.ApplicationSetServiceClient, error) {
	conn, err := NewConnection(c.address, c.timeoutSeconds, &c.tlsConfig)
	if err != nil {
		return nil, nil, err
	}
	return conn, applicationset.NewApplicationSetServiceClient(conn), nil
}
...
func NewApplicationSetClientset(address string, timeoutSeconds int, tlsConfig TLSConfiguration) Clientset {
